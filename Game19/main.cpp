...
int main() {
    ...
    for (i = 1; i < line - 1; i++) {
        for (j = 1; j < col - 1; j++) {
            map[i][j] = randomizeNumber (territoriesQuantity);
        }
    }
    ...
    while (life > 0) {
        ...
        do {
            ...
            std::cout << "Character's position at the array: " << i << "  " << j;
        ...
        foeNumber = randomizeNumber (foesQuantity);
        territoryMessage (foeNumber, map[i][j], life);
