...
void ambushCombat (int &life, int &foeLife) {
    ...
    std::cout << "Throw your sword at him, and use your knife for the combat. / Draw your sword and prepare for the fight. Choose: ";
    ...
    if (action < 11) {
        ...
        dealsDamagePenalty(characterDamageChanged, foeLife, characterPenalty, characterPenaltyText, characterPenaltyTextQuantity);
        ...
        if (answer == "tumble") {
            ...
            if (action < 7) {
                std::cout << "You tried to parry a giant sword with a knife and didn't succeeded. He removed a big chunk of flesh out of your chest.\n";
                ...
                std::cout << "You incredibly parried a giant sword with your knife (surprising everyone). Then smashed his knee with a kick, breaking it.\n";
        ...
        if (life > 0 && foeLife > 0) {
            ...
            std::cout << "Try to evade with your body to the left. And cut his neck thereafter. / Try to shift his sword's trajectory using your knife, and cut his neck thereafter. Choose: ";
            words9 = {"body", "knife"};
            ...
            if (answer == "body") {
                if (action < 4) {
                    ...
                    std::cout << "You successfully evade his attack. As you had to be fast, you only sliced his back instead of his neck.\n";
                    characterDamageChanged = characterDamage * 9;
                    foePenalty.push_back(4);
                ...
                if (action < 7) {
                    std::cout << "Your knife couldn't deflect his giant sword. He pierced your pelvis, hurting you strongly.\n";
                    ...
                    std::cout << "You surprisingly deflected his giant sword with your sword (surprising everyone). As you had to be fast, you only sliced his back instead of his neck.\n";
        ...
        if (life > 0 && foeLife > 0) {
            std::cout << "You two still standing, there is blood running across the grass . You two are facing each other. He starts to get closer.\n";
            std::cout << "Run toward him and at the last moment, jump and try to pierce his heart. / Walk toward him and suddenly stick your knife in his heart. Choose: ";
            words10 = {"run", "walk"};
            answer = checkWords(2, words7);
            action = randomizeNumber(10);
            if (answer == "run") {
                if (action < 3) {
                    ...
                    characterPenaltyTextQuantity.push_back(1);
                    receivesDamagePenalty(foeDamageChanged, life, foePenalty, foePenaltyText, foePenaltyTextQuantity);
                    characterDamageChanged = characterDamage * 22;
                    ...
                    characterDamageChanged = characterDamage * 24;
                    foePenalty.push_back(6);
                    foePenaltyText.push_back("Without heart");
                    foePenaltyTextQuantity.push_back(1);
                    dealsDamagePenalty(characterDamageChanged, foeLife, characterPenalty, characterPenaltyText, characterPenaltyTextQuantity);
...
int characterPenalties(std::vector<int> characterPenalty, std::vector<std::string> characterPenaltyText, std::vector<int> characterPenaltyTextQuantity) {
    ...
    if (totalCharacterPenalty < 1) {
        ...
        for (i = 0; i <= characterPenalty.size() - 1; i++) {
...
int foePenalties(std::vector<int> foePenalty, std::vector<std::string> foePenaltyText, std::vector<int> foePenaltyTextQuantity) {
    ...
    if (totalFoePenalty < 1) {
        ...
        for (i = 0; i <= foePenalty.size(); i++) {
            std::cout << foePenaltyTextQuantity[i] << " - " << foePenaltyText[i] << " - Dealing " << foePenalty[i]
                      << " less damage.\n";
        }
    }
