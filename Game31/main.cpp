...
int main() {
    ...
    int territoriesQuantity, line, col, i, j, n, m, x, y, positionToReplace, movement, life, foesQuantity, foeNumber, charPosition, insurmountable;
  ...
  */
    ...
    for (i = 1; i < line - 1; i++) {
        for (j = 1; j < col - 1; j++) {
            ...
            if (insurmountable == line - 2) {
                std::cout << "Check: " << insurmountable;
                positionToReplace = randomizeNumber (line - 2);
                for (j = 1; j < col - 1; j++) {
                    if (j == positionToReplace) {
                        map[i][j] = randomizeNumber (3) + 2;
                    }
                }
            }
        }
