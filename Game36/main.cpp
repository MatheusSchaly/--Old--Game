 ...
 */
    ...
    for (i = 1; i < line - 1; i++) {
        for (j = 1; j < col - 1; j++) {
            ...
            while (insurmountable >= 3) {
                mapHelper = randomizeNumber (4);
                if (mapHelper == 1 && (map[i - 1][j] == 1 || map[i - 1][j] == 2)) {
                    map[i - 1][j] = randomizeNumber(3) + 2;
                    territoriesChangesQuantity += 1;
                    insurmountable -= 1;
                }
                if (mapHelper == 2 && (map[i + 1][j] == 1 || map[i + 1][j] == 2)) {
                    map[i + 1][j] = randomizeNumber(3) + 2;
                    territoriesChangesQuantity += 1;
                    insurmountable -= 1;
                }
                if (mapHelper == 3 && (map[i][j - 1] == 1 || map[i][j - 1] == 2)) {
                    map[i][j - 1] = randomizeNumber(3) + 2;
                    territoriesChangesQuantity += 1;
                    insurmountable -= 1;
                }
                if (mapHelper == 4 && (map[i][j + 1] == 1 || map[i][j + 1] == 2)) {
                    map[i][j + 1] = randomizeNumber(3) + 2;
                    territoriesChangesQuantity += 1;
                    insurmountable -= 1;
                }
    ...
    std::cout << "\nQuantity of territories changed: " << territoriesChangesQuantity << std::endl;
